...
//    std::cout << "i am here!\n";
    ...
    std::cout << "i am here! :::: " << uthread_get_tid() << std::endl;

    std::cout << "i am here! :::: uthread_get_quantums ::::" << uthread_get_quantums(uthread_get_tid()) << std::endl;

    wait_next_quantum();

    std::cout << "i am here!22222222\n";
    ...
    std::cout << "i am here!33333\n";

    wait_next_quantum();
    std::cout << "i am here!444444444\n";
...
void thread1()
{
    int t3 = uthread_spawn(thread3); // first half
...
int main()
{
    ...
    std::cerr << "did we got here?1" << std::endl;

    for (int i = 0; i < ARRAY_SIZE; i++)
    {
        array[i] = rand() % 4862;
    }
    std::cerr << "did we got here?2" << std::endl;
    ...
    std::cerr << "did we got here?3" << std::endl;

    while (thread_status[t1] == RUN || thread_status[t2] == RUN)
    {}
    std::cerr << "did we got here?4" << std::endl;


    merge(0, N, place_holder0);

    std::cerr << "did we got here?5" << std::endl;
